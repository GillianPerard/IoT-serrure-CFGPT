...
module.exports = {
    ...
    /** DASHBOARD **/
    //Creation d'un nouvel utilisateur
    dashNewAccount: function (req, res) {
      var _password = req.param("password");
      var _mail = req.param("email");
      var _name = req.param("lastname");
      var _firstname = req.param("firstname");

      if (!_mail || !_password || !_name || !_firstname){
      ...
      Users.create({ firstname: _firstname, lastname: _name, email: _mail, password: _password }).exec(function (err, created) {
        ...
        req.flash('valid', "L'utilisateur '" + _firstname + " " + _name + "' a été créé avec succès. Veuillez vous connecter. :)");
        return res.redirect('/connexion');
      });
    },

    // Récupère les groupes d'un user à partir de son token en session
    dashGetGroupsByToken: function (req, res) {
      var _tokenUser = req.session.userToken;

      //Si pas de token, on drop
      if (!_tokenUser)  return res.notFound();
      ...
      Users.findOneByRefreshToken(_tokenUser).exec(function (err, currUser) {
        ...
        GroupUsers.find({ user: currUser.id }).populate('group').exec(function (err, groupUsers) {
          ...
          return res.view('groups/trousseaux', {
            _user:currUser,
